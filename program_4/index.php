...
$.get( "http://mwsu-webdev.xyz/api/api.php/products?order=id&page=1,10")
  .done(function( data ) {
	...
	var prods=data.products.records;
	for(var i=0;i<prods.length;i++)
	{
		var col_head1 = "<tr>";
		console.log(prods[i][4]);
		var str = prods[i][4];
		console.log(str);
		var newstr = str.replace('~', '80');
		console.log(newstr);
		for(var j=0;j<5;j++)
		{
		...
		if(j<4)
		{
			col_head1 = col_head1 + "<td>"+prods[i][j]+"</td>";
		}
		else
		{
			col_head1 = col_head1 + "<td><img src="+newstr+"></td>";
		}
					
		}
		col_head1 = col_head1 + "<td style=\"width: 36px;\"></td></tr>";
		$('tbody').append(col_head1);
