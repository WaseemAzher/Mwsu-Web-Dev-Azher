...
<div class="modal fade" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
               <h3> Are you Sure you want to delete the below Product</h3>
            ...
            <div class="modal-footer">
                ...
                <a class="btn btn-danger btn-ok" id="deleteId" data-dismiss="modal">Delete</a>
            </div>
        </div>
    </div>
</div>	
...
(function($) {
    ...
    */
    ...
    function loadTableData(page, page_size,sort,order) {
		...
		console.log("http://107.170.48.163/api/api.php/products?order="+sort+"&page=" + page + "," + page_size);
        $.get("http://107.170.48.163/api/api.php/products?order="+sort+order+"&page=" + page + "," + page_size)
        ...
        .done(function(data) {
            ...
            for (var i = 0; i < cols.length; i++) {
                ...
                col_head += "<th class=\"headClass downsort\"> <i class=\"fa fa-caret-down sort-up \" aria-hidden=\"true\"></i> " + cols[i];
            ...
            for (var i = 0; i < records.length; i++) {
                ...
                for (var j = 0; j < records[i].length; j++) {
                    ...
                    rows = rows + "<td class='cols'>" + records[i][j] + "</td>";
                }
	...
	$(".downsort").click(function() {	
		   ...
		   {			   
			...
			$('.btnDelete').click(function(){
                ...
                $tds1 = $row.find("td:nth-child(1)").text(), // Finds the closest row <tr> 
                $tds = $row.find("td:nth-child(3)").text();  // Finds the 3nd <td> element		
				$('#descId').text($tds);				
				$('#Id').val($tds1);				
				$('.modal-dialog').show();
			});
			...
			$('#deleteId').click(function(){
				...
				$('.modal-dialog').hide();		
                $.ajax({
				url: ('http://107.170.48.163/api/api.php/products/'+$id),
				type: 'DELETE',				
				traditional:true,
				dataType: 'json',
    ...
    function getTotalPages(){
        $.get("http://107.170.48.163/api/api.php/products")
